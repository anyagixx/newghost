...
## Keywords
wss, websocket, secure-websocket, proxy, internet-access, iroh, sendme, nat-traversal

## Annotation
n0wss is a Web Secure Socket (WSS) proxy service based on the sendme/iroh technology stack. The goal is to let a local client connect to a remote WSS endpoint and route outbound internet traffic through it so proxy-aware applications can use the tunnel as a generic upstream proxy.
